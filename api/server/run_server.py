...
def prepare_search_fast_response(proteins_with_euclid, search_time):
    results = [
        {
            'object_id': protein_with_euclid[0],
            'euclid': protein_with_euclid[1],
        }
        for protein_with_euclid in sorted(proteins_with_euclid, key=lambda x: x[1])
    ]

    return {'results': results, 'search_time': search_time}


@app.route('/search_fast')
def execute_searching_fast():
    query = request.args.get('query')
    LOG.info(f"Searching fast for query: '{query}'")

    proteins_with_euclid, search_time = search_fast(query)
    search_response = prepare_search_fast_response(proteins_with_euclid, search_time)

    response = make_response(jsonify(search_response))
