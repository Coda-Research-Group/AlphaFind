...
compute_scores() {
    ...
    QUERY_PROTEIN_PATH="/eph/untared_proteins/${QUERY_ID}/AF-${QUERY_ID}-F1-model_v3.cif.gz"
    DATASET_EXCTRACTED_PROTEIN_PATH="/eph/untared_proteins/${QUERY_ID}/AF-${PROTEIN_ID}-F1-model_v3.cif.gz"

    /home/protein-db-dev/USalign "${QUERY_PROTEIN_PATH}" "${DATASET_EXCTRACTED_PROTEIN_PATH}" -outfmt 2 | tail -n 1 | awk -F ' ' '{print $3,$5,$8,$9,$11}' >"/eph/partial_scores/${QUERY_ID}/${PROTEIN_ID}.txt"
}
